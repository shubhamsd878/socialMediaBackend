...
const sign_up = async (req, res) => {
  ...
  try {
        ...
        data.save((err, result) => {
          if (err) {
            ...
            name.save((err, nameresult) => {
              if (err) {
                console.log(
                  "something went wrong during saving name, err: ",
                  err
                );
                return res
                  .status(500)
    ...
    } else res.status(201).json({ message: "email already registered" });
  } catch (error) {
...
const updatePassword = (req, res) => {
  ...
  if (!auth || !newPassword) {
    ...
    return res
      .status(500)
      .json({ message: false, err: "authtoken or newPassword are invalid!" });
  }
