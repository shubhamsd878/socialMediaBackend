...
router.get('/totalFollowers', async (req, res) => {
    ...
    let prevTotalFollowers = await totalFollowers.findById(uid)
        .catch(err => { return res.status(500).json({ message: 'somthign went wrong', err }) })
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        }, { upsert: true }, async (err, response) => {
            if (err) return res.status(500).json({ message: 'something went wrong', err })
            else {
                // ---------------- increment follower count in totalFollowers

                // fetching prevFollower, if null then intializing to 0
                let prevTotalFollowers = await totalFollowers.findById(targetUid).catch(errr => { return res.status(500).json({ message: 'somthign went wrong', errr }) })
                if (prevTotalFollowers == null) {
                    prevTotalFollowers = 0
                }
                else {
                ...
                totalFollowers.findByIdAndUpdate(targetUid,
                    { totalFollowers: prevTotalFollowers + 1 },
                    { upsert: true }, (errr, resp) => {
                        if (errr) return res.status(500).json({ message: 'somthign went wrong', errr })
