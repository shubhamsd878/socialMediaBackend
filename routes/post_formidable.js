...
router.get("/fetch", async (req, res) => {
  ...
  console.log("fetchPost > payload > ", "skip: ", skip, "limit: ", limit);
  ...
  // -----------------------------------------------------------
  let followingArr = await followingModel
    .find({ uid }, "following -_id")
    .catch((err) => {
  ...
  if (followingArr.length == 0)
    return res.status(200).json({ message: "no friends of user" });

  followingArr = followingArr[0].following;
  if (followingArr.length == 0)
    return res.status(201).json({ status: 201, message: "no friends of user" });

  // -------------------------- Count Total Results ---------------------------------

  let totalResult = await postSchema
    .count({ uid: { $in: followingArr } })
    .catch((err) => {
      return res.status(500).json({ message: "something went wrong", err });
    });
  ...
  let result = await postSchema
    .find({ uid: { $in: followingArr } }, {}, { sort: { date: -1 } })
  ...
  console.log("fetchPost > posts: ", result.length);
...
router.get("/fetchcurrent", async (req, res) => {
  ...
  let totalResult = await postSchema.count({ uid: targetUid }).catch((err) => {
    return res.status(500).json({ message: "something went wrong", err });
  });

  let result = await postSchema
    .find({ uid: targetUid }, {}, { sort: { date: -1 } })
    .skip(skip)
    .limit(limit)
    .populate({ path: "uid", populate: { path: "name" } })

    .catch((err) => {
      console.error("error in fetching current user posts: ", err);
      return res.status(500).json({ message: "something went wrong", err });
    });
